...
float SceneElement::lightPos[4*MAX_NUM_LIGHTS] =
	{
		4.1875, 2.66, -0.8125, 1.0,
		-4.3125, 2.56, 2.1195, 1.0,
		1.0, 5.0, 1.0, 0.0
	};
...
float SceneElement::globalAmbient[] = { 0.75, 0.75, 0.70 };
...
void SceneElement::establishMaterial(const PhongMaterial& matl)
{
	...
	glUniform1i(shaderIF->ppuLoc("useTexture"), useTexture);
}
...
float SceneElement::establishSeason(){
	...
	last_time = glfwGetTime();
	float season = -cos(.05 * total_time) * 0.666 + 0.5;
	glUniform1f(shaderIF->ppuLoc("season"), season);
	globalAmbient[0] = 0.45 - (season * 0.25);
	globalAmbient[1] = 0.45 - (season * 0.25);
	globalAmbient[2] = 0.35 - (season * 0.15);
	lightStrength[6] = 1 - (season * 0.7);
	lightStrength[7] = 1 - (season * 0.7);
	lightStrength[8] = 0.95 - (season * 0.65);
	if(season < 0.5){
...
void SceneElement::establishTexture(GLuint texID, bool useAlpha,int mode)
{
	...
	useTexture = true;
	glUniform1i(shaderIF->ppuLoc("useTextureAlpha"), useAlpha);
